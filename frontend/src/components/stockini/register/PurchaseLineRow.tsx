...
export function PurchaseLineRow({ line, lineNumber, onChange, onDelete }: Props) {
  ...
  const handleProductSelect = (product: Product) => {
    const puHt = round3(Number(product.purchasePrice));
    onChange(
      recalculateLine({
        ...line,
  ...
  return (
    <tr className="h-9 border-b border-border/30 group hover:bg-slate-50/80">
      {/* N° */}
      ...
      <td className={`min-w-[55px] ${CELL}`}>
        <input
          ...
          onChange={(e) =>
            update({ tvaPercent: Math.max(0, Math.min(100, Number(e.target.value) || 0)) })
          }
          className={NUM_INPUT}
        />
      </td>
      ...
      <td className={`min-w-[60px] ${CELL}`}>
        <input
          ...
          onChange={(e) =>
            update({ remisePercent: Math.max(0, Math.min(100, Number(e.target.value) || 0)) })
          }
          placeholder="0"
          className={NUM_INPUT}
        />
      ...
      {/* Net HT */}
      <td className={`min-w-[80px] px-2 text-xs text-right tabular-nums font-medium text-text-primary ${CELL}`}>
        {fmt(line.netHt)}
      </td>

      {/* Net TTC */}
      <td className={`min-w-[85px] px-2 text-xs text-right tabular-nums font-semibold text-text-primary ${CELL}`}>
        {fmt(line.netTtc)}
      </td>
