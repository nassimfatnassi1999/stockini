...
function KpiCard({ label, value, icon: Icon, color, bg, positive, description, tooltip }: KpiCardProps) {
  const isPos = value >= 0;
  return (
    <div className={cn('rounded-xl border border-border bg-card p-3 shadow-sm', 'flex items-start gap-3 min-h-[80px]')}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start gap-1">
          ...
          {tooltip && (
            <span title={tooltip} aria-label={tooltip} className="shrink-0 text-text-muted">
              <Info size={12} />
            </span>
          )}
        </div>
...
function buildGlobalCards(summary: CashSummary, periodLabel: string): KpiCardProps[] {
  ...
  const benefitTooltip = "Somme des ventes nettes HT après remises, moins le coût historique des produits vendus et l’impact des avoirs.";
  return [
    ...
    { label: 'Bénéfice semaine', value: summary.salesPeriods?.week.grossProfit ?? summary.profitSemaine, icon: CalendarDays, color: summary.profitSemaine >= 0 ? 'text-sky-600' : 'text-red-500', bg: 'bg-sky-50', positive: true, tooltip: benefitTooltip },
    { label: 'Bénéfice mois', value: summary.salesPeriods?.month.grossProfit ?? summary.profitMois, icon: CalendarDays, color: summary.profitMois >= 0 ? 'text-violet-600' : 'text-red-500', bg: 'bg-violet-50', positive: true, tooltip: benefitTooltip },
    { label: 'Bénéfice année', value: summary.salesPeriods?.year.grossProfit ?? summary.profitAnnee, icon: TrendingUp, color: summary.profitAnnee >= 0 ? 'text-amber-600' : 'text-red-500', bg: 'bg-amber-50', positive: true, tooltip: benefitTooltip },
  ];
}
...
function buildAccountCards(acc: AccountSummary, solde: number, periodLabel: string, isCash: boolean): KpiCardProps[] {
  ...
  return [
    ...
    { label: `Bénéfice brut réel — ${periodLabel}`, value: acc.profit, icon: acc.profit >= 0 ? TrendingUp : TrendingDown, color: acc.profit >= 0 ? 'text-emerald-600' : 'text-red-500', bg: acc.profit >= 0 ? 'bg-emerald-50' : 'bg-red-50', positive: true, description: 'Ventes nettes HT − coût des produits vendus', tooltip: benefitTooltip },
    { label: 'Bénéfice semaine', value: acc.profitSemaine, icon: CalendarDays, color: acc.profitSemaine >= 0 ? 'text-sky-600' : 'text-red-500', bg: 'bg-sky-50', positive: true, tooltip: benefitTooltip },
    { label: 'Bénéfice mois', value: acc.profitMois, icon: CalendarDays, color: acc.profitMois >= 0 ? 'text-violet-600' : 'text-red-500', bg: 'bg-violet-50', positive: true, tooltip: benefitTooltip },
    { label: 'Bénéfice année', value: acc.profitAnnee, icon: TrendingUp, color: acc.profitAnnee >= 0 ? 'text-amber-600' : 'text-red-500', bg: 'bg-amber-50', positive: true, tooltip: benefitTooltip },
  ];
}
