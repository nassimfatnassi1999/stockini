...
export function calculateSalesLine(input: SalesLineCalculationInput) {
  ...
  const marginPercentOnCost = purchaseCostHt.gt(0) ? marginAmountHt.div(purchaseCostHt).mul(100) : new Decimal(0);
  ...
  return {
    ...
    netMarginPercent: marginPercentOnCost.toNumber(), marginPercentOnCost: marginPercentOnCost.toNumber(),
    ...
    discountAmount: discountAmountUnitHt.mul(quantity).toNumber(), discountAmountHt: discountAmountUnitHt.mul(quantity).toNumber(),
    marginAmount: quantity.gt(0) ? marginAmountHt.div(quantity).toNumber() : 0,
    marginAmountHt: marginAmountHt.toNumber(), purchaseCostHt: purchaseCostHt.toNumber(),
    totalHt: lineNetHt.toNumber(), lineNetHt: lineNetHt.toNumber(), taxAmount: vatAmount.toNumber(), vatAmount: vatAmount.toNumber(),
    totalTtc: lineTtc.toNumber(), lineTtc: lineTtc.toNumber(),
  };
}

export function calculateSalesTotals(lines: SalesLineCalculation[], fiscalStamp: number | string = 0) {
  const totalHt = lines.reduce((s, l) => s.plus(l.lineNetHt), new Decimal(0));
  const totalVat = lines.reduce((s, l) => s.plus(l.vatAmount), new Decimal(0));
  const totalDiscountHt = lines.reduce((s, l) => s.plus(l.discountAmountHt), new Decimal(0));
  const totalPurchaseCostHt = lines.reduce((s, l) => s.plus(l.purchaseCostHt), new Decimal(0));
  const totalMarginHt = totalHt.minus(totalPurchaseCostHt);
  const stamp = Decimal.max(0, decimal(fiscalStamp));
  return { totalHt: salesRound3(totalHt), totalVat: salesRound3(totalVat), totalDiscountHt: salesRound3(totalDiscountHt),
    totalPurchaseCostHt: salesRound3(totalPurchaseCostHt), totalMarginHt: salesRound3(totalMarginHt),
